	...
	return result.operations
		.filter(
			(operation) =>
				...
				typeof operation.destinationDirPath === "string",
		)
		.map((operation) => {
			const noteName = basename(operation.path)
			const openPath = normalizePathSeparators(
				join(operation.destinationDirPath!, noteName),
			)
			return {
				id: operation.path,
	...
	const moveNotesWithAI = useCallback(
		async (targetEntries: WorkspaceEntry[]) => {
			...
			try {
				...
				toast.error(getTranslation().aiAgent.moveFailed, {
			...
			const t = getTranslation().aiAgent
			toast.custom(
				(toastId) =>
